...
#   * Пояснение «≈» в подвале: в строке «Казна: N [+X≈ / -Y≈]» (HUD карты и
#     верхняя полоса города) маркер означает средние за последнее окно
#     прибыль и расход, пересчитанные в секунду (CityData.
#     get_treasury_flow_text). Раньше строка показывала только баланс, и
#     пояснение нужно было к одной секции прибыли; теперь «≈» стоит с обеих
#     сторон, поэтому подвал рисуется всегда, когда тултип отрисован.
...
func show_treasury_tooltip(mouse_pos: Vector2, balance: int, planned_income: Dictionary, expense_snapshot: Dictionary, window_sec: float, keep_position: bool = false):
    ...
    # Пояснение нужно и к секции прибыли, и к секции расходов: маркер «≈»
    # теперь стоит с обеих сторон в строке «Казна: N [+X≈ / -Y≈]» (HUD карты и
